# # host:
# # db.stnxjphrwhbwhxkggtvs.supabase.co
...
# Load from environment (e.g. Render or .env)
DB_CONFIG = {
...
def add_player_to_db(name):
    ...
    try:
        cur.execute("INSERT INTO players (name) VALUES (%s) ON CONFLICT DO NOTHING RETURNING id", (name,))
        conn.commit()
...
def get_all_ongoing_games_from_db():
    conn = get_connection(); cur = conn.cursor()
    cur.execute("SELECT id, players, teams, match_count, matches, created_at, match_scores FROM games WHERE status='ongoing' ORDER BY id DESC")
...
def save_completed_game(game_data):
    ...
    """, (
        ...
        json.dumps(game_data["matches"]),
        json.dumps(game_data["results"]),
        datetime.fromisoformat(game_data["created_at"]),
        datetime.fromisoformat(game_data["ended_at"])
    ))
    conn.commit()
    conn.close()
...
def get_all_completed_games():
    conn = get_connection(); cur = conn.cursor()
    cur.execute("SELECT id, players, teams, match_count, matches, results, created_at, ended_at FROM completed_games ORDER BY ended_at DESC")
    rows = cur.fetchall(); conn.close()
    return [
        {
            "id": r[0],
            "players": r[1], "teams": r[2], "match_count": r[3],
            "matches": r[4], "results": r[5],
            "created_at": r[6].isoformat(), "ended_at": r[7].isoformat()
        } for r in rows
    ]

def mark_game_as_completed(game_id):
    conn = get_connection(); cur = conn.cursor()
    cur.execute("UPDATE games SET status='completed' WHERE id = %s", (game_id,))
...
def save_player_stats(stats, game_id, created_at):
    ...
    for s in stats:
        cur.execute("""
            ...
            (player, played, won, lost, point_diff, game_id, created_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, (
            s["name"], s["played"], s["won"], s["lost"],
            float(s["pointDifferential"]), game_id,
            datetime.fromisoformat(created_at)
        ))
    conn.commit(); conn.close()

# --- Rankings & Partner Stats...
# (Rewrite your existing logic here using JSONB queries similarly)

# --- Deletion (removes completed + stats + game)
def delete_game(game_id):
    conn = get_connection(); cur = conn.cursor()
    cur.execute("DELETE FROM completed_games WHERE id = %s", (game_id,))
    cur.execute("DELETE FROM player_stats WHERE game_id = %s", (game_id,))
